...
public class Hospital extends Building implements Comparable<Hospital> {
    private int number;
    private final List<Doctor> doctors = new ArrayList<>();

    public Hospital(Builder builder) throws IllegalArgumentException {
        super(builder.address);
        this.number = builder.number;
        for (Doctor doctor : doctors)
            if (doctor.getHospitalNumber() != number)
                throw new IllegalArgumentException("Doctor " + doctor.getName() + " " +
                        doctor.getSurname() + " not attach to hospital");
